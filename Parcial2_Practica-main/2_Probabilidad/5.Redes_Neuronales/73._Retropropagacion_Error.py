...
class RedNeuronalMulticapa:
    ...
    def backward(self, entrada, etiquetas, salida):
        """
        Implementación del algoritmo de retropropagación del error.

        Args:
            entrada (np.ndarray): Lote de datos de entrada.
            etiquetas (np.ndarray): Lote de etiquetas verdaderas (one-hot encoded).
            salida (np.ndarray): Salida predicha de la red para el lote de entrada.
        """
        m = entrada.shape[0]  # Número de ejemplos en el lote

        # --- Paso 1: Calcular el error en la capa de salida ---
        delta_salida = salida - etiquetas

        # --- Paso 2: Calcular el gradiente de los pesos de la capa oculta a la salida ---
...
if __name__ == "__main__":
    ...
    y_one_hot = np.eye(2)[y]  # Convertir etiquetas a
